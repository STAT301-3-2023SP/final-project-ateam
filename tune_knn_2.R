...
# create workflow ----
## k nearest neighbors model ----
knn_workflow_log_sqrt <- workflow() %>% 
  add_model(knn_mod) %>% 
...
knn_tune_log_sqrt <- tune_grid(
  knn_workflow_log_sqrt,
  resamples = cars_fold,
  grid = knn_grid,
  control = control_grid(save_pred = TRUE,
                         save_workflow = TRUE,
...
knn_tictoc_log_sqrt <- tibble(model = time_log[[1]]$msg,
                         runtime = time_log[[1]]$toc - time_log[[1]]$tic)


save(knn_tune_log_sqrt, knn_tictoc_log_sqrt,
     file = "results/tuning_knn_rel.rda")
