...
We tuned at fit 9 models in our exploration. Aside from the null model, we added additional parameters to our other models to further improve their performance. Each of these graphs shows specific parameters for each model compared to the roc_auc metric to determine when the models perform best.
- As mentioned in the Assessment Measures section, the roc_auc curve provides a visual representation of the relationship between the true positive rate (sensitivity) and the false positive rate (1-specificity). 
...
```
In this model, the peak of the curve moves towards the top-left corner, which indicates that this model has a relatively low rate of falsely identifying negative cases as positive. Also, the peak of the curve is approaching the upper-left corner, which means this model has a higher proportion of actual positive cases that were correctly classified as positive. These results show that this model is proficient in its predictions and performs well. 

For this model, it seems that it has the best roc_auc score when it has around 15 randomly selected predictors. When this occurs, the roc_auc score for this model is relatively high, but otherwise, it doesn't perform very well.
...
```
This model looks very similar to the random forest model, and produces the same advantages as that model. This model performs best with a higher number of predictors.
...
```
This model has the best roc_auc score when it has lower levels of regularization. Regularization is a method that is used to prevent overfitting. At it's best, this model has low regularization, which means means the model is less restricted, which cause overfitting.
...
```
This model has the best roc_auc score when it has 5 model terms. The model terms refer to the individual functions that make up the MARS model, so in this case, the more components the MARS model has, the better its roc_auc performance will be.
...
```
This model contains a multitude of results, which means this model is a poor representation of a good roc_auc score. 

In general, it seems that the model performs better with a lower of hidden units, which means it has a less complex model structure with less patterns and representations. 
...
```
For both degrees of interaction, this model has the highest roc_auc score with 5 model terms. 
...
autoplot(svm_poly_tune_ks, metric = "roc_auc")
```
Similar to the model above, within each kernel, this model has extremely varied results. The different scale factors do not have a cohesive cost parameter that has the best roc_auc score, so overall, this model has a relatively low roc_auc score. 
...
=======
When plotted using the importance recipe, the random forest model performs best with a larger number of predictors. 
>>>>>>> 72850496f28667b0921006dcf6c5c038bb48000b
...
The final model we will use is the random forest with the kitchen sink recipe. We are not surprised, as random forest models do well in managing outliers and in preventing overfitting. Also, as seen before, the random forest However, we are surprised that the kitchen sink recipe performed better than the importance recipe. Again, it is likely due to the number folds and repeats used for each recipe and model.
