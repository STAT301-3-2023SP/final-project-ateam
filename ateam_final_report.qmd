...
Overall, the final model performed well but did not fit our purposes. We were looking to see what variables make a car exchangeable. However, our final model ended up being a kitchen sink model that used all of the variables as predictors. The recipes in which we used variable selection methods, the models did not perform as well. We think that we need more variables in the dataset to choose from, as right now there are only 20.
