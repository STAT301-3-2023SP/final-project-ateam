...
```
This model has the best roc_auc score when it has lower levels of regularization. Regularization is a method that is used to prevent overfitting. At it's best, this model has low regularization, which means means the model is less restricted, which can cause overfitting.
...
```
This model performance varies as the amount of regularization changes. There is not a clear trend of what amount of regularization leads to a higher roc_auc score.
...
In each of these graphs, the results vary heavily among each of the scale factors, with the higher scale factors performing better. Generally, cost does not seem to affect the roc_auc score. This means that although this model can perform well, it's overall roc_auc score is relatively low.
...
```
The boosted tree model with the relationship recipe has the best roc_auc score as the number of randomly selected predictors increases. Smaller minimum node size seems to improve the roc_auc score. The learning rate does not seem to affect thr roc_auc score.
...
```
The elastic net model has the best roc_auc score when the amount of regularization is around 1e-03. Generally, a range of 0 to 1e-03 leads to a higher roc_auc score. In this range, the roc_auc score is highest, and the amount of regularization means the model is relatively more proficient at preventing overfitting.
...
```
This model performs similarly to the previous on with the highest roc_auc score occurring when the amount of regularization remains between 0 and 1e-03.
...
```
For both degrees of interaction, this model has the highest roc_auc score as the number of model terms increases. 2 degrees of intereaction also lead to a higher roc_auc score.
