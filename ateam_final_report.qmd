...
```
This model looks very similar to the random forest model, and produces the same advantages as that model. This model performs best with a higher number of predictors and a smaller minimal node size.
...
```
Similar to the model above, the boosted tree model has the best roc_auc score as the number of randomly selected predictors increases. Smaller minimum node size seems to improve the roc_auc score.
